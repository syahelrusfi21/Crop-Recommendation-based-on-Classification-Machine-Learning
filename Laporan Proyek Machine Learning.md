...
### Problem Statement
1. Bagaimana cara menentukan jenis tanaman yang paling sesuai untuk ditanam berdasarkan karakteristik tanah dan iklim agar hasil pertanian optimal?
2. Bagaimana membangun sistem klasifikasi tanaman yang akurat dan dapat diandalkan untuk mendukung pengambilan keputusan berbasis data dalam sektor pertanian?

### Goals
1. Membangun model klasifikasi machine learning yang dapat memprediksi jenis tanaman optimal berdasarkan fitur seperti kandungan Nitrogen (N), Fosfor (P), Kalium (K), suhu udara, kelembapan, pH tanah, dan curah hujan.
2. Mengevaluasi performa berbagai algoritma klasifikasi untuk memilih model terbaik dengan akurasi prediksi tertinggi dan kestabilan performa yang baik.
...
### Encoding Label
Label yang berupa teks dikonversi menjadi format numerik menggunakan `LabelEncoder` agar dapat diproses oleh model machine learning.
...
Pembagian dilakukan sebelum normalisasi untuk menghindari kebocoran data uji.

### Normalisasi Fitur
Fitur dinormalisasi menggunakan `MinMaxScaler`. Normalisasi hanya dilakukan pada data latih, lalu transformasi yang sama diterapkan ke data uji. Normalisasi dilakukan untuk mengubah skala semua fitur numerik ke rentang 0–1 agar seimbang dalam perhitungan algoritma.
